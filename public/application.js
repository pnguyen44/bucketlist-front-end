...
var onGetBucketList = function onGetBucketList() {
  api.getBucketList().then(ui.getBucketListSuccess).catch(ui.getBucketListFailure);
};

var onPostBucketList = function onPostBucketList(event) {
  event.preventDefault();
  var data = getFormFields(this);
  if (data.listitem.name.trim().length) {
    $('.message-form').html('');
    api.postBucketList(data).then(ui.postBucketListSuccess).catch(ui.postBucketListFailure);
  } else {
    $('.message-form').html('Name field is required');
  }
};

var getItemId = function getItemId(button) {
  var element = $(button).parent().parent().children()[0];
  var elementId = $(element).attr('data-id');
  return elementId;
};

var getItemName = function getItemName(button) {
  var element = $(button).parent().prev().children()[0];
  var itemName = $(element).attr('data-item-name');
  return itemName;
};

var getItemLocation = function getItemLocation(button) {
  var element = $(button).parent().prev().children()[1];
  var itemLocation = $(element).attr('data-item-location');
  return itemLocation;
};

var getItemCategory = function getItemCategory(button) {
  var element = $(button).parent().prev().children()[2];
  var itemCategory = $(element).attr('data-item-category');
  return itemCategory;
};

var getItemRating = function getItemRating(button) {
  var element = $(button).parent().prev().children()[3];
  var itemRating = $(element).attr('data-item-rating');
  return itemRating;
};

var getItemStatus = function getItemStatus(button) {
  var element = $(button).parent().prev().children()[4];
  var itemStatus = $(element).attr('data-item-status');
  return itemStatus;
};
...
var onDeleteItem = function onDeleteItem(event) {
  ...
  var data = getItemId(deleteButton);
  api.deleteItem(data).then(function () {
    ui.deleteItemSuccess(deleteButton);
  }).catch(ui.deleteItemFailure);
};

var getCurrentData = function getCurrentData(editButton) {
  var itemName = getItemName(editButton);
  var itemLocation = getItemLocation(editButton);
  var itemCategory = getItemCategory(editButton);
  var itemRating = getItemRating(editButton);
  var itemStatus = getItemStatus(editButton);
...
var isChecked = $('#incomplete-button').is(':checked');
...
var onUpdateItem = function onUpdateItem(event) {
  event.preventDefault();
  var data = getFormFields(this);
  var elementId = store.editedItemId;
  if (data.listitem.name.trim().length) {
    $('.message-form').html('');
    api.updateItem(elementId, data).then(ui.updateItemSuccess).catch(ui.updateItemFailure);
  } else {
    $('.message-form').html('Name field is required');
  }
};
...
module.exports = {
  ...
  onGetBucketList: onGetBucketList,
  onPostBucketList: onPostBucketList
};
...
var getOneBucketItem = function getOneBucketItem(id) {
  return $.ajax({
...
var getBucketList = function getBucketList() {
  return $.ajax({
...
var postBucketList = function postBucketList(data) {
  return $.ajax({
    url: config.apiOrigin + '/listitems/',
    method: 'POST',
    headers: {
...
var updateItem = function updateItem(elementId, data) {
  return $.ajax({
...
module.exports = {
  ...
  postBucketList: postBucketList,
  deleteItem: deleteItem,
  updateItem: updateItem,
  getOneBucketItem: getOneBucketItem
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
/* harmony default export */ __webpack_exports__["default"] = (__webpack_require__.p + "77ff91d7fa0317186a8eff6c9da3920e.eot");
...
var bucketlistEvents = __webpack_require__(6);
var yelpEvents = __webpack_require__(37);
...
var onSignUp = function onSignUp(event) {
  event.preventDefault();
  var data = getFormFields(this);
  var isMatchPasswords = void 0;
  var allFilledIn = void 0;
  clearMessage();
...
var clearFormInput = function clearFormInput() {
  $('#email-address').val('');
  $('#password').val('');
...
var store = __webpack_require__(1);
var showList = __webpack_require__(20);
var api = __webpack_require__(8);

var getBucketListSuccess = function getBucketListSuccess(data) {
  ...
  if (store.listitems.length === 0) {
    ...
    var newList = store.listitems.filter(function (item) {
      return item.status === store.status;
    });
    var showListHTML = showList({ listitems: newList });
    $('#incomplete-button').show();
...
var getBucketListFailure = function getBucketListFailure() {
  $('.add-bucketList-message').html('Error. Please try again.');
};

var postBucketListSuccess = function postBucketListSuccess(data) {
  $('form').trigger('reset');
  $('#modal-add').modal('hide');
  api.getBucketList().then(getBucketListSuccess).catch(getBucketListFailure);
};

var postBucketListFailure = function postBucketListFailure() {
  $('.add-bucketList-message').html('Error. Please try again.');
};

var deleteItemSuccess = function deleteItemSuccess(deleteButton) {
...
module.exports = {
  getBucketListSuccess: getBucketListSuccess,
  getBucketListFailure: getBucketListFailure,
  postBucketListSuccess: postBucketListSuccess,
  postBucketListFailure: postBucketListFailure,
...
var onYelpSearch = function onYelpSearch(event) {
  event.preventDefault();
  var data = getFormFields(this);
  if (data.location.trim().length && data.term.trim().length) {
    $('.yelp-message').html('');
    api.getYelpResults(data).then(ui.getYelpResultsSuccess).catch(ui.getYelpResultsFailure);
  } else {
...
var getItemName = function getItemName(button) {
  var itemName = $(button.target).parent().siblings('.resultName').attr('data-item-name');
  return itemName;
};

var getItemLocation = function getItemLocation(button) {
  var itemLocation = $(button.target).parent().siblings('.resultLocation').attr('data-item-location');
  return itemLocation;
};

var getCurrentData = function getCurrentData(addButton) {
  $('.message-form').html('');
  var itemName = getItemName(addButton);
  var itemLocation = getItemLocation(addButton);
  $('#input-yelp-category').val('');
  $('#input-yelp-rating').val('');
...
var getYelpResults = function getYelpResults(data) {
  return $.ajax({
...
var store = __webpack_require__(1);
var showResult = __webpack_require__(40);

var getYelpResultsSuccess = function getYelpResultsSuccess(data) {
  store.yelpresults = data.yelpresults;

  if (data.yelpresults.length) {
    $('.yelp-message').text('');
    var showResultHTML = showResult({ yelpresults: store.yelpresults });
...
var getYelpResultsFailure = function getYelpResultsFailure() {
  $('.yelp-message').text('Search failed. Please try again.');
  $('#search-content').html('');
};

module.exports = {
  getYelpResultsSuccess: getYelpResultsSuccess,
  getYelpResultsFailure: getYelpResultsFailure
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "<div class=\"col-6 box-yelp-result\">\n  <div class=\"col-3 pull-left container-result-pic\">\n    <img class=\"yelpImage\" src=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.image_url : stack1), depth0))
    + "\"></img>\n  </div>\n  <div class=\"col-3 pull-left container-result-info\" data-id=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\">\n    <div class=\"resultName\" data-item-name=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.name : stack1), depth0))
    + "\">Name: "
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.name : stack1), depth0))
    + "</div>\n    <div class=\"resultLocation\" data-item-location=\""
    + alias2(alias1(((stack1 = ((stack1 = blockParams[0][0]) != null ? stack1.location : stack1)) != null ? stack1.city : stack1), depth0))
    + "\">Location: "
    + alias2(alias1(((stack1 = ((stack1 = blockParams[0][0]) != null ? stack1.location : stack1)) != null ? stack1.city : stack1), depth0))
    ...
    + "\">Visit Site: "
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.name : stack1), depth0))
    + "</a></div>\n    <br>\n    <div><button class=\"add-result-button\" data-toggle=\"modal\" data-target=\"#modal-add\">Add To Bucket List!</button></div>\n  </div>\n</div>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.yelpresults : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "");
},"useData":true,"useBlockParams":true});
...
"use strict";
Object.defineProperty(__webpack_exports__, "__esModule", { value: true });
/* harmony default export */ __webpack_exports__["default"] = (__webpack_require__.p + "3abf0f95c0a69984006e3778b65e513f.ttf");
